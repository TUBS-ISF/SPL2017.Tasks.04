...
public class ConsoleUi implements MenuHelper {
	...
	Map<Integer, ConverterTypePlugin> chosenConverterFeature;
...
//		this.konverterMenuList = konverterMenuList;
		...
		chosenConverterFeature = new HashMap<Integer, ConverterTypePlugin>();
...
//		}
		...
		for (ConverterTypePlugin konverterPlugin : konverterList) {
			String entry = "# " +counter+ ") " + konverterPlugin.getName();
			chosenConverterFeature.put(counter, konverterPlugin);
			typeMenuEntries.add(entry);
			//konverterPlugin.setMenuHelper(this);
			counter += 1;
		}
	...
	public void showConversionTypeMenu(int chosenFeature) throws NotSupportedException {
		...
		ConverterTypePlugin converter = chosenConverterFeature.get(chosenFeature);
		//uiPlugin.showMenu();
		GenericConverterMenu menu = new GenericConverterMenu(this);
		menu.showMenu(converter, converter.getConverterTypesEnum());
	}

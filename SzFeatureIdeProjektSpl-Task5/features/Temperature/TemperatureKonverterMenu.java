...
public class TemperatureKonverterMenu implements ConsoleUiPlugin {
	...
	public void showMenu() throws NotSupportedException {
		...
		TemperatureConverter.TemperatureType[] temperatureTypes = TemperatureConverter.TemperatureType.values();
		if (temperatureTypes.length < sourceTypeInt) {
			System.err.println("sourceType not found: " + sourceTypeInt);
			System.exit(-1);
		}
		
		TemperatureConverter.TemperatureType sourceType = temperatureTypes[sourceTypeInt];
		...
		TemperatureConverter.TemperatureType destType = temperatureTypes[destTypeInt];
